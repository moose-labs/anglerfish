...
module anglerfish::anglerfish_tests {
    ...
    fun setup(scenario: &mut Scenario): (PrizePoolCap, PhaseInfoCap, PoolCap, LoungeCap, RoundRegistryCap) {
    ...
    fun test_lounge_create_and_claim() {
        ...
        let claimed_coin = lounge::claim<TEST_COIN>(&mut lounge_registry, lounge_number, ts::ctx(&mut scenario));
    ...
    fun test_pool_deposit_and_redeem() {
        ...
        let (pool_total_shares,user_shares, reserves) = {
            ...
            (pool::get_total_shares(pool), pool::get_user_shares(pool, USER1), pool::get_reserves(pool).value())
        };
    ...
    fun test_full_lottery_cycle() {
        ...
        ts::next_tx(&mut scenario, ADMIN);
        {
            ...
            prize_pool_cap.start_new_round(&round_registry_cap,&phase_info_cap, &mut phase_info, &mut round_registry , &clock, ts::ctx(&mut scenario));
        ...
        assert_eq(round::get_prize_amount(&round), pool_registry.get_total_prize_reserves_value<TEST_COIN>());
        ...
        // Claim protocol fee
        ts::next_tx(&mut scenario, ADMIN);
        let protocol_fee = prize_pool::claim_protocol_fee<TEST_COIN>(&prize_pool_cap, &mut prize_pool, ts::ctx(&mut scenario));
