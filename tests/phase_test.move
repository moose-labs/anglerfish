...
module red_ocean::phase_test {
    ...
    use red_ocean::phase;
    use red_ocean::phase::{
        init_for_testing, PhaseInfo, PhaseInfoCap,
    };
    ...
    fun build_phase_test_suite(sender: address): (Scenario, Clock) {
    ...
    fun test_capability_cannot_be_taken_by_unauthorized_user() {
        ...
        let (mut scenario, clock)  = build_phase_test_suite(AUTHORITY);
    ...
    fun test_cannot_phase_changes() {
        ...
        let (mut scenario, mut clock) = build_phase_test_suite(AUTHORITY);
